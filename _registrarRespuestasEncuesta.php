<?php 
    ...
    $idUsuario = $_SESSION['id_user'];
        ...
        //Validar Usuario ya tenga una pregunta registrada
        $conocerEncuestaRealizada = "SELECT * FROM respuestasencuestas WHERE idEncuestas = $idEncuesta and idUsuario = $idUsuario";
        ...
        if($f_conocerEncuestaRealizada > 0){
            ...
            //No hay respuestas ingresada se ingresaran
            foreach ($_POST["pregunta"] as $key => $value){
                $consultaInsertarRespuestas =  "INSERT INTO respuestasencuestas(idPreguntasEncuestas,idEncuestas,idUsuario,fechahora,respuesta) VALUES('$key','$idEncuesta','$idUsuario','$fecha $hora','$value');";
                $iny_consultaInsertarRespuestas = mysqli_query($mysqli,$consultaInsertarRespuestas,$link) or mysqli_error($mysqli);
                header("Location:encuestas.php");
            }
        }
...
*/
    ...
    */
    ...
    if($oper == "add" ){
        ...
        $consultaAgregar = "insert into encuestas (idUsuario,fechahora,status, titulo) 
    ...
    if($oper=="edit"){
        $idEncuesta = $_GET['i'];
        $consultaEditar = "update encuestas set  status = '$statusEncuesta', titulo = '$titulo' where idEncuesta = $idEncuesta;";
