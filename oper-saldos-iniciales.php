...
<html lang="es">
  <head>
    <title><?php echo $_SESSION['nombre-sitio']; ?> :: <?php echo $operacion; ?> Saldos Iniciales</title>
    ...
    <div class="row">
      <div class="col-xs-12">
        <ol class="breadcrumb">
          ...
          <li><a href="panel-saldos-iniciales.php">Panel de Administracion</a></li>
          <li class="active"><?php echo $operacion; ?> Saldos Iniciales</li>
        </ol>
      </div>
    </div>
  </div>
<div class="container">
  <div class="row">
    <div class="col-xs-12">
      <h4><?php echo $operacion; ?> Saldos Iniciales</h4><br>
    </div>
  </div>
  <?php 
    ...
    $conocerUsuarioSinSaldoInicial = "select id_user,nombre,no_casa from user where not exists(select id_user from saldos_iniciales where saldos_iniciales.id_user = user.id_user) and level=0 order by no_casa asc;";
    $INY_CONOCER_USUARIOS_SIN_SALDO_INICIAL = mysqli_query($mysqli,$conocerUsuarioSinSaldoInicial,$link)or die(mysqli_error($mysqli));

  ?>
  <form action="_oper_saldos_iniciales.php?oper=<?php echo $typeOper;?> " method="POST" name="frm_User" class="form-horizontal" enctype="multipart/form-data">
    <div class="row">
          ...
          <div class="col-xs-12">
            <div class="form-group">
              ...
              <div class="col-md-8 col-xs-9">
                  ...
                  <?php
                    while($fUsuarioSin = mysqli_fetch_array($INY_CONOCER_USUARIOS_SIN_SALDO_INICIAL)){
                      echo "<option value='".$fUsuarioSin[0]."'>".$fUsuarioSin[2]." [".$fUsuarioSin[1]."]</option>";
                    }
                  ?>
        ...
        <div class="row">
          <div class="col-xs-12">
            <div class="form-group">
              ...
              <div class="col-md-8 col-xs-9">
                <input name="descripcion" id="tituloInpt" class="form-control" value= "Saldo Inicial" disabled type="text" required placeholder="Descripcion de Gasto" />
              </div>
            </div>
          </div>
        </div>
