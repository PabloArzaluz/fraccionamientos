<?php
  ...
  $current_page_admin = "comprobantes-gastos";
  ...
  <head>
    ...
    function cambiarMes(){
      if(document.getElementById('generaFecha').options[document.getElementById('generaFecha').selectedIndex].value != ""){
        ...
        window.location = "panel-comprobantes-gastos.php?fecha="+fechaRequerida;
      }
...
<br>
  <div class="row">
    <div class="col-xs-9">
      <a href="oper_comprobante.php?agre=add" class="btn btn-success"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span> Cargar Comprobante</a>
    </div>
    <div class="col-xs-3 text-right">

      <?php 
        $consulta_mes = mysql_query("SELECT distinct year(fecha),month(fecha) FROM comprobantes WHERE MONTH(fecha) order by fecha desc;",$link) or die(mysql_error());
  ...
  <div class="row">
    <div class="col-xs-12">
      <div class="alert alert-success" role="alert">
        <span class="glyphicon glyphicon-ok" aria-hidden="true"></span> Comprobante Cargado Correctamente
      </div>
    </div>
  </div>
  <?php } ?>
...
//echo $fechaRequerida;
//echo $fecha_entre;
  $consulta_noti = mysql_query("SELECT * FROM comprobantes where fecha BETWEEN '$fechaRequerida' and '$fecha_entre';",$link) or die(mysql_error());
  while($arr_comentarios = mysql_fetch_array($consulta_noti)){ 
            ...
            elseif ($arr_comentarios[5]=="jpg") {
              ...
              echo "<td><a href='delete-comprobante.php?id=$arr_comentarios[0]&f=$fechaRequerida' class='btn btn-danger btn-xs'>Eliminar</a></td></tr>";
            }
          ?>
</table>
    </div>
